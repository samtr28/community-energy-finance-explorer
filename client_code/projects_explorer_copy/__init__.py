from ._anvil_designer import projects_explorer_copyTemplate
from anvil import *
...
class projects_explorer_copy(projects_explorer_copyTemplate):
  ...
  def form_show(self, **event_args):
    ...
    self.project_map.layout.map = dict(
      center=dict(lat=57, lon=-97), zoom=2, style="carto-voyager"
    )
  ...
  def _select_index(self, idx: int):
    ...
    if 0 <= idx < len(rows):
      ...
      if self._hi_card:
        self._hi_card.role = (
          (self._hi_card.role or "").replace("card-highlight", "").strip()
        )

      row.project_card.role = (
        (row.project_card.role or "") + " card-highlight"
      ).strip()
