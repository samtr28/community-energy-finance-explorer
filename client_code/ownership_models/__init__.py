...
class ownership_models(ownership_modelsTemplate): 
  ...
  def apply_filters(self):
    ...
    owner_types = self.owner_types_dd.selected
    ...
    if owner_types:
      kwargs["owner_types"] = owner_types
    ...
    if owner_types:
      for ot in owner_types:
        chips.append({'text': f'Owner Type: {ot}', 'tag': ('owner_types', ot)})
  ...
  def remove_filter(self, filter_type, value):
    ...
    elif filter_type == 'owner_types':
      current = list(self.owner_types_dd.selected)
      current.remove(value)
      self.owner_types_dd.selected = current
  ...
  def owner_types_dd_change(self, **event_args):
    """This method is called when the selected values change"""
    self.schedule_filter_update()
