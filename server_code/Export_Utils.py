...
# ── Banner background ──
BANNER_BG_COLOR     = '#f0f0f0'    # light grey
...
# ── Chart title (top-left of banner, replaces source heading) ──
TITLE_TEXT_SIZE     = 28           # pt
TITLE_TEXT_COLOR    = '#002754'    # brand navy
TITLE_TOP_MARGIN    = 24           # px from top of banner
...
# ── Logo in bottom strip ──
LOGO_STRIP_HEIGHT   = 40           # px
LOGO_STRIP_WIDTH    = 200          # px
...
def add_logo_and_filters_pil(img_bytes, active_filters, chart_title='',
  ...
  Bottom strip:
    - Source citation left, logo right, both vertically centred.
  ...
  text_stack_h = (
    TITLE_TOP_MARGIN
    + TITLE_TEXT_SIZE
    + 16                    # gap between title and filter block
    + filter_block_h
  )
  banner_h = max(
  ...
  draw.text(
    (FILTER_LEFT_MARGIN, TITLE_TOP_MARGIN),
    chart_title,
    fill=TITLE_TEXT_COLOR,
    font=_load_font(TITLE_TEXT_SIZE, bold=True)
  )

  # ── Banner logo: top-right ──
  if logo:
  ...
  try:
    strip_logo = _resize_logo(
      Image.open(data_files[logo_filename]).convert('RGBA'),
      LOGO_STRIP_HEIGHT, LOGO_STRIP_WIDTH
    )
    sl_w, sl_h = strip_logo.size
    lx = w - sl_w - LOGO_STRIP_PADDING
    ly = strip_y + (BOTTOM_STRIP_HEIGHT - sl_h) // 2
    canvas.paste(strip_logo, (lx, ly), strip_logo)
  except Exception as e:
    print(f"Strip logo skipped: {e}")
...
def export_figure_from_bytes(img_b64, active_filters, filename='chart_export.png',
  ...
  Args:
    ...
    chart_title:    title string for the top of the export banner
    project_count:  number of projects in the filtered dataset (reserved for future use)
