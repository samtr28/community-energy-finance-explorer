...
def apply_filters(df, provinces=None, proj_types=None, stages=None,
  ...
  # Normalise project_scale to plain stripped strings regardless of
  # whether the column came from a Categorical or object dtype source.
  # This is the root cause of Mega rows being silently dropped from
  # df_owners_filtered.
...
def get_all_ownership_charts(provinces=None, proj_types=None, stages=None,
  ...
  df_owners_filtered = apply_filters(df_owners, provinces, proj_types, stages, indigenous_ownership, project_scale)
    ...
    # ── DEBUG ──────────────────────────────────────────────────
  print("\n=== df_owners_filtered (Mega rows) ===")
  mega_owners = df_owners_filtered[df_owners_filtered['project_scale'] == 'Mega (> $100M)']
  print(mega_owners[['record_id', 'owner_name', 'owner_type', 'owner_category', 
                    'owner_percent', 'total_cost']].to_string())
  
  print("\n=== df_raw_filtered owners field (all rows) ===")
  for _, row in df_raw_filtered.iterrows():
    owners = row.get('owners') or []
    print(f"  record {row['record_id']} ({row.get('project_scale')}):")
    for o in owners:
      print(f"    category={o.get('owner_category')!r:25} "
            f"type={o.get('owner_type')!r:30} "
            f"percent={o.get('owner_percent')!r}")

  # Find the 4 Mega records by elimination — we know they're records NOT in the 29 shown
  known_records = {1,4,7,8,9,11,12,16,28,31,33,34,35,36,37,43,64,66,68,80,81,85,89,95,99,102,103,104,106}
  print("\n=== Records in df_raw NOT in the 29 shown ===")
  for _, row in df_raw.iterrows():
    if row['record_id'] not in known_records:
      print(f"  record {row['record_id']}: scale={row['project_scale']!r}")
      print(f"    owners: {row.get('owners')}")
      print(f"    total_cost: {row.get('total_cost')}")  
  # ── END DEBUG ──────────────────────────────────────────

  print("\n=== MEGA PROJECT RAW DATA ===")
  for _, row in df_raw_filtered[df_raw_filtered['project_scale'] == 'Mega (> $100M)'].iterrows():
    print(f"\nRecord {row['record_id']}:")
    print(f"  owners: {row.get('owners')}")
    print(f"  bottlenecks: {row.get('bottlenecks')}")
  ...
  # --- Charts built from the flattened owners frame ---
  owners_keys = ['ownership_treemap', 'scale_pies', 'indigenous_pie',
                 'ownership_boxplot', 'ownership_tiers_histogram']
  if df_owners_filtered.empty:
    owners_charts = {k: _empty() for k in owners_keys}
  else:
    owners_charts = {
      ...
      'ownership_boxplot':         create_ownership_boxplot_internal(df_owners_filtered),
      'ownership_tiers_histogram': create_ownership_tiers_histogram_internal(df_owners_filtered),
    }

  # --- Charts built from the raw per-response frame ---
  raw_keys = ['lollipop_chart', 'all_financing_heatmap', 'collaboration_heatmap',
              'single_owner_breakdown', 'multi_owner_semicircles']
  if df_raw_filtered.empty:
    raw_charts = {k: _empty() for k in raw_keys}
  else:
    raw_charts = {
      'lollipop_chart':          create_governance_bottlenecks_internal(df_raw_filtered),
      'all_financing_heatmap':   create_ownership_all_financing_heatmap_internal(df_raw_filtered),
      'collaboration_heatmap':   create_collaboration_heatmap_internal(df_raw_filtered),
      'single_owner_breakdown':  create_single_owner_breakdown_internal(df_raw_filtered),
      'multi_owner_semicircles': create_multi_owner_semicircles_internal(df_raw_filtered),
    }

  merged = {**owners_charts, **raw_charts}
  return {k: apply_display_template(v) for k, v in merged.items()}
...
def create_ownership_tiers_histogram_internal(df_owners):
  ...
  TIER_COLORS = {'<25%': '#c9e4ca', '25–50%': '#87bba2', '51–74%': '#55828b',
  ...
  for col_i, cat in enumerate(cat_order, start=1):
    ...
    fig.update_xaxes(tickangle=45, tickfont=dict(size=9), showticklabels=True, row=1, col=col_i)
...
def create_governance_bottlenecks_internal(df):
  ...
  grouped = pd.DataFrame(rows).groupby(['bottleneck', 'ownership_type']).size().reset_index(name='count')
  ...
  for _, row in df.iterrows():
    ...
    for oc, pct_sum in cat_percents.items():
      for fc in finance_cats:
        pairs.append({
          'record_id': rid, 'owner_category': oc, 'finance_category': fc,
...
def create_ownership_all_financing_heatmap_internal(df):
  ...
  for fi, row_label in enumerate(count_pivot.index):
    for oi, col_label in enumerate(count_pivot.columns):
      ...
      annotations.append(dict(
        x=col_label, y=row_label, text=f'<b>{int(val)}</b>', showarrow=False,
        xref='x', yref='y',
  ...
  fig.update_layout(
    ...
    annotations=annotations, margin=dict(l=0, b=0, t=50, r=0),
    font=dict(family=FONT_FAMILY, size=FONT_SIZE, color=FONT_COLOR),
...
def create_collaboration_heatmap_internal(df):
  ...
  for yl in ordered:
    for xl in ordered:
      ...
      if val <= 0: continue
      annotations.append(dict(
        x=xl, y=yl, text=f'<b>{int(val)}</b>', showarrow=False, xref='x', yref='y',
  ...
  fig.update_layout(
    ...
    annotations=annotations, margin=dict(l=0, r=0, t=50, b=0),
    font=dict(family=FONT_FAMILY, size=FONT_SIZE, color=FONT_COLOR),
...
def create_single_owner_breakdown_internal(df):
  ...
  fig = px.bar(
    ...
    text=None,  # ensure no text labels
  )
...
def create_multi_owner_semicircles_internal(df):
  """
    One semicircle per project with 2+ owners. Each slice = one owner,
    sized by ownership %, coloured by owner_type (a shade of its owner_category colour).
    Threshold: sum > 90 (lenient — handles partial/rounded ownership data).
    """
  KNOWN = set(CATEGORY_COLOUR_SCHEME.keys())

  # Build owner_type → colour map from all types present in the data
  pair_set = set()
  for _, row in df.iterrows():
  ...
  for _, row in df.iterrows():
    ...
    valid = [
      o for o in owners
      if (o.get('owner_percent') or 0) > 0          # ← original simple check, not pd.to_numeric
      and (o.get('owner_category') or 'Other') in KNOWN
    ]
    if len(valid) < 2:
      continue
    values = [float(o['owner_percent']) for o in valid]
    if sum(values) <= 90:                              # ← restored original lenient threshold
      continue
    projects.append({
  ...
  for i, p in enumerate(projects):
    ...
    labels      = p['types'] + ['']
    vals        = p['values'] + [total]               # last slice = invisible "back half"
    ...
    # Legend entries — one per owner_type actually present
  for t in types_used:
