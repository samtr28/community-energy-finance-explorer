...
def create_ownership_scale_pies_internal(df_owners):
  ...
  for i, scale in enumerate(scales):
    ...
    # ── View A: Sum of ownership % (current — large stakes dominate) ──
    share_grouped = sub.groupby(['owner_type', 'owner_category'], as_index=False)['owner_percent'].sum()

    # ── View B: Number of projects per owner type ──
    count_grouped = (sub.groupby(['owner_type', 'owner_category'])['record_id']
      .nunique().reset_index()
      .rename(columns={'record_id': 'project_count'}))

    # Sort both consistently — category order, then owner type
    for d in (share_grouped, count_grouped):
    ...
    # In-slice text labels
    share_grouped['percentage']   = (share_grouped['owner_percent'] /
                                     share_grouped['owner_percent'].sum()) * 100
    share_grouped['text_display'] = share_grouped['percentage'].apply(
      lambda x: f'{x:.1f}%' if x >= 5 else ''
    )
    count_grouped['text_display'] = count_grouped['project_count'].apply(
      lambda c: f'{c}' if c >= 1 else ''
    )
    ...
    domain = dict(x=[x_start, x_end], y=[0.22, 1.0])

    # Trace A — Ownership share (visible)
    fig.add_trace(go.Pie(
      ...
      hovertemplate='<b>%{label}</b><br>%{percent} of ownership<extra></extra>',
      showlegend=False, visible=True,
    ))

    # Trace B — Project count (hidden)
    fig.add_trace(go.Pie(
      labels=count_grouped['owner_type'],
      values=count_grouped['project_count'],
      domain=domain, title=title_style,
      marker=dict(colors=[owner_type_colors[ot] for ot in count_grouped['owner_type']]),
      sort=False, direction='clockwise',
      textinfo='text', text=count_grouped['text_display'],
      hovertemplate='<b>%{label}</b><br>%{value} projects (%{percent})<extra></extra>',
      showlegend=False, visible=False,
    ))

  # ── Custom gradient legend (same as before) ──
  shades_per_category = {}
  ...
  fig.update_layout(
    ...
    updatemenus=[dict(
      type='buttons', direction='left',
      buttons=[
        dict(label='By Ownership Share', method='update', args=[{'visible': vis_share}]),
        dict(label='By Project Count',   method='update', args=[{'visible': vis_count}]),
      ],
      pad={'r': 10, 't': 10}, showactive=True,
